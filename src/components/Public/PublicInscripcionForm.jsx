...
import QR from "../../assets/codigoQR/Global.jpeg";
...
export default function PublicInscripcionForm() {
  ...
  const getQRByCategory = (categoriaId, categorias) => {
    ...
    if (nombreCategoria.includes("federado")) {
      ...
      nombreCategoria.includes("campesino") ||
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        showError(
          "Error cargando categorías y equipos. Verifica que el servidor esté funcionando."
        );
        setCategorias([]);
        setEquipos([]);
      } finally {
  ...
  const validateFormData = () => {
    ...
    if (!form.comprobante || !form.foto_anverso || !form.foto_reverso) {
      showError(
        "Debe subir el comprobante de pago y las fotos del CI (anverso y reverso)"
      );
      return false;
    }
    ...
    if (birthYear < 1900 || birthYear > 2011) {
      showError(
        "Solo se admiten participantes nacidos entre los años 1900 y 2011"
      );
      return false;
    }
    ...
    if (age < 18 && !form.autorizacion) {
      showError(
        "Para participantes menores de 18 años es obligatorio subir la autorización firmada por los padres/tutores"
      );
      return false;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (age < 18) {
        showWarning(
          "Procesando inscripción de menor de edad. Verificando documentación..."
        );
      }
      ...
      if (error.code === "ECONNABORTED") {
        errorMessage =
          "La inscripción está tardando demasiado. Por favor, inténtalo de nuevo.";
      } else if (error.response?.status === 413) {
        errorMessage =
          "Los archivos son demasiado grandes. Por favor, reduce el tamaño e inténtalo de nuevo.";
  ...
  const currentAge = form.fecha_nacimiento
    ? calculateAge(form.fecha_nacimiento)
    : null;
  const currentBirthYear = form.fecha_nacimiento
    ? getBirthYear(form.fecha_nacimiento)
    : null;
  const isMenorDeEdad = currentAge && currentAge < 18;
  // CORREGIDO: Permitir años desde 1900 hasta 2011
  const isValidBirthYear =
    currentBirthYear && currentBirthYear >= 1900 && currentBirthYear <= 2011;

  // Función para obtener el precio según la categoría
  const getPriceByCategory = (categoryTitle) => {
    const prices = {
      campesinos: 20,
      damas: 20,
      masters: 20,
    ...
    // Precio por defecto si no se encuentra la categoría
    return 20;
  };
    ...
    <div className="bg-white rounded-2xl shadow-xl p-4 sm:p-6 md:p-8 mx-2 sm:mx-4 md:mx-auto max-w-6xl">
      <div className="text-center mb-6 sm:mb-8">
        ...
        <div className="mt-6 sm:mt-8 p-3 sm:p-4 bg-red-50 rounded-lg border border-red-200">
          <p className="text-xs sm:text-sm text-lime-700 leading-relaxed">
            <strong>Nota:</strong> Esperar mientras se carga el formulario y
            cuando se mande la inscripción
          </p>
        </div>
        ...
        <div className="mt-4 p-3 sm:p-4 bg-blue-50 rounded-lg border border-blue-200">
          <p className="text-xs sm:text-sm text-blue-700 leading-relaxed">
            <strong>Requisito de Edad:</strong> Solo se admiten participantes
            nacidos entre los años 1900 y 2011 (ambos incluidos).
          </p>
        </div>
        ...
        <div className="mt-4 p-3 sm:p-4 bg-red-50 rounded-lg border border-red-200">
          <p className="text-xs sm:text-sm text-red-700 leading-relaxed">
            <strong>Nota:</strong> Si desea inscribir a un menor de edad. Los
            Organizadores NO SE RESPONSABILIZAN por los accidentes del ciclista
            antes, durante y después de la Competencia, siendo de total
            responsabilidad del ciclista y de los padres y/o tutores del menor
            de edad, cualquier inconveniente que sufrieran y declaro estar en
            conformidad y de pleno acuerdo con todos los términos establecidos
            en la reglamentación de la competencia descritos en la convocatoria,
            deberá adjuntar una autorización firmada por los padres y/o tutores
            del menor de edad.
          </p>
        </div>
      </div>
      ...
      {!dataLoading && (
        <>
          ...
          {message && (
            <div
              className={`mb-4 sm:mb-6 p-3 sm:p-4 rounded-lg flex items-start gap-3 text-sm sm:text-base ${
                messageType === "success"
          ...
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
                ...
                {form.fecha_nacimiento && (
                  <div className="mt-1">
                    <p className="text-xs text-gray-600">
                      Edad: {calculateAge(form.fecha_nacimiento)} años (Año:{" "}
                      {getBirthYear(form.fecha_nacimiento)})
                    </p>
                    {!isValidBirthYear && (
                      <p className="text-xs text-red-600 font-medium">
                        ❌ No cumple con el requisito de edad (Nacidos entre
                        1900 y 2011)
                      </p>
                    )}
                      ...
                      <div className="mt-1 p-2 bg-orange-50 rounded border border-orange-200">
                        <div className="flex items-start gap-2">
                          ...
                          <p className="text-orange-700">
                            <strong>Requerido:</strong> Documento firmado por
                            padres/tutores autorizando la participación del
                            menor de edad.
                          </p>
                        </div>
                      </div>
            ...
            <div className="text-center text-gray-600 text-sm sm:text-base items-center mt-6">
              {form.categoria_id ? (
                (() => {
                  ...
                  return qrData ? (
                    <div>
                      ...
                      <div className="mt-4 p-3 sm:p-4 bg-blue-50 rounded-lg border border-blue-200">
                        <p className="text-xs sm:text-sm text-blue-700 leading-relaxed">
                          <strong>Categoría {qrData.title} debe pagar:</strong>{" "}
                          {getPriceByCategory(qrData.title)}Bs
                        </p>
                      </div>
                      <div className="mt-4 p-3 sm:p-4 bg-blue-50 rounded-lg border border-blue-200">
                        <p className="text-xs sm:text-sm text-blue-700 leading-relaxed">
                          <strong>
                            Los extranjeros deben pagar OBLIGATORIAMENTE:
                          </strong>{" "}
                          70Bs
                        </p>
                      </div>
                ...
                <div className="p-4 bg-gray-50 rounded-lg border border-gray-200">
                  <p className="text-gray-500">
                    Selecciona una categoría para ver el código QR de pago
                    correspondiente
                  </p>
                </div>
              )}
            ...
            <div className="text-center pt-4 sm:pt-6">
              <button
                disabled={
                  loading || (form.fecha_nacimiento && !isValidBirthYear)
                }
                type="submit"
          ...
          <div className="mt-6 sm:mt-8 p-3 sm:p-4 bg-blue-50 rounded-lg border border-blue-200">
            <p className="text-xs sm:text-sm text-blue-700 leading-relaxed">
              <strong>Nota:</strong> Tu inscripción será verificada una vez que
              subas todos los documentos requeridos. El número de dorsal será
              asignado por los organizadores y te será comunicado
              posteriormente.
            </p>
          </div>
        </>
