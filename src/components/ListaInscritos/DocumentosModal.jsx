...
import { X, Download, FileText, CreditCard, Shield, AlertCircle } from "lucide-react";
...
const DocumentosModal = ({
  ...
  const fetchDocumentUrl = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/api/inscripciones/archivo/${encodeURIComponent(documentPath)}`,
        {
          headers: {
      ...
      console.error('Error obteniendo URL del documento:', err);
  ...
  const getDocumentTitle = (type) => {
    switch (type) {
      case 'anverso':
        return 'CI/DNI - Anverso (Frente)';
      case 'reverso':
        return 'CI/DNI - Reverso (Atrás)';
      case 'autorizacion':
        return 'Autorización de Padres/Tutores';
      default:
        return 'Documento';
    }
  };
  ...
  const getDocumentIcon = (type) => {
    switch (type) {
      case 'anverso':
      case 'reverso':
        return <CreditCard className="w-4 h-4 sm:w-5 sm:h-5 text-blue-500 flex-shrink-0" />;
      case 'autorizacion':
        return <Shield className="w-4 h-4 sm:w-5 sm:h-5 text-purple-500 flex-shrink-0" />;
      default:
        return <FileText className="w-4 h-4 sm:w-5 sm:h-5 text-gray-500 flex-shrink-0" />;
    }
  };
    ...
    if (loading) {
      return (
        <div className="flex flex-col items-center justify-center py-8 sm:py-12 space-y-4">
          ...
          <p className="text-sm sm:text-base text-gray-600">Cargando documento...</p>
        </div>
      );
    }
      ...
      return (
        <div className="flex flex-col items-center justify-center py-8 sm:py-12 space-y-4">
          ...
          <p className="text-gray-600 text-center">
            No se ha subido {getDocumentTitle(documentType).toLowerCase()} para este participante.
          </p>
        </div>
      );
    }
      ...
      case 'image':
        return (
          <div className="flex justify-center p-2">
            <img
              src={documentUrl}
              alt={getDocumentTitle(documentType)}
        ...
        return (
          <div className="flex flex-col items-center justify-center py-8 sm:py-12 space-y-4">
            ...
            <p className="text-xs sm:text-sm text-gray-500 text-center px-4">
              Archivo: {documentPath.split('/').pop()}
            </p>
            <button
